...
8. generator Unit gate は **statement coverage 90%**（`covermode=atomic`、`-shuffle=on`、`-count=1`）。除外は Composition Root（`internal/composition/**`）・CLI Driving Adapter（`cmd/**`）・build tag 付き local 実物 suite・Broad Integration 以上。`error.go` / `names.go` / `constants.go` を名前では除外しない。secret なし Narrow Integration（`apps/generator/test/` の build tag なし）は production code のカバー計測分母に含める（実プロセス境界を持つ処理は SU では埋まらないため。`testing-strategy/coverage.md` §3・§4）。build tag 付き local 実物 suite と Broad Integration は計測に含めない。Integration **gate の実行対象**分離は別軸で `docs/decisions/2026-08-26T17-42-00-docs-infra-test-discussion.md` を正とする。本件の判断の正は `docs/decisions/2026-08-30T11-52-00-feature-generator-cursorcli-narrow-integration.md`
