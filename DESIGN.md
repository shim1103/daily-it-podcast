...
1. 分類は **file 名**に出す（`sociable_unit` / `narrow_integration` / `broad_integration` / `contract` / `system`）。`integration` 一語で兼ねない。Generator System 分類語は `system`
...
6. runner: Playback = Vitest、Generator = `go test`
...
8. playback Unit gate: branch coverage（全体 100%、外部境界層は glob で 90%）。設定は `apps/playback/vitest.config.mjs`
...
12. Integration **gate**（pre-push / GHA Integration）: secret なし Narrow + Broad。System / 本番 produce は gate 外（収集・入口の正は code と `DEPLOY.md`）
