...
# description: 原稿 API が prompt どおりの draft を返す率を計測する dispatch 専用 test を実行する。
...
echo "draft-rate: generator (go) api=${DRAFT_RATE_API:-cursor}"
(
  cd "$root/apps/generator"
  go test -v -tags="system ratemeasure" -run TestDraftRate -timeout 60m -count=1 ./test/system/...
)
