...
#          表示用に API / RUNS / PASS_THRESHOLD / PROMPT_VARIANT を env で受ける（任意）。
...
{
  echo "### 原稿 API draft rate（api=${API:-cursor}）"
  echo ""
  echo "- api=${API:-cursor} runs=${RUNS:-?} threshold=${PASS_THRESHOLD:-?} variant=${PROMPT_VARIANT:-?}"
  if [ -f "$log" ]; then
