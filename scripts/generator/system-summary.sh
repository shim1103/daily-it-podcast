...
# @ensure top-level test（Test 名に "/" を含まない = サブテストでない）の pass/fail/skip を数えて summary へ書く。
...

{
  ...
  if [ -f "$jsonl" ]; then
    ...
    pass=$(grep -E '"Action":"pass"' "$jsonl" | grep -E '"Test":"[^"/]+"' | wc -l | tr -d ' ')
    fail=$(grep -E '"Action":"fail"' "$jsonl" | grep -E '"Test":"[^"/]+"' | wc -l | tr -d ' ')
    skip=$(grep -E '"Action":"skip"' "$jsonl" | grep -E '"Test":"[^"/]+"' | wc -l | tr -d ' ')
    total=$((pass + fail + skip))
    echo "- top-level test: pass ${pass} / fail ${fail} / skip ${skip}（total ${total}）"
    if [ "${fail}" -gt 0 ]; then
      echo ""
      echo "FAIL:"
      grep -E '"Action":"fail"' "$jsonl" | grep -oE '"Test":"[^"/]+"' | sort -u
    fi
  else
