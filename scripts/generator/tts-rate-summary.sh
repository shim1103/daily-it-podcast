...
#          表示用に RUNS / CALL_GAP / RETRY_BACKOFF_BASE / RETRY_BACKOFF_MAX / PASS_THRESHOLD を env で受ける（任意）。
...

{
  ...
  echo ""
  echo "- runs=${RUNS:-?} callGap=${CALL_GAP:-?} backoffBase=${RETRY_BACKOFF_BASE:-?} backoffMax=${RETRY_BACKOFF_MAX:-?} threshold=${PASS_THRESHOLD:-?}"
  if [ -f "$log" ]; then
