...
# description: wrangler experimental local S3 を peer とする generator Narrow Integration を実行する。
# @require リポジトリ内から呼ぶ。Go と apps/playback の wrangler（npm install）がある。
# @ensure `-tags r2locals3` の Writer / Lookup NI が exit 0。
# @invariant test-unit に混ぜない。本 script の go test だけが -tags r2locals3 を持つ（Decision §1-5）。
# @invariant 本番 credential を読まない。起動失敗を黙って skip して緑にしない。
set -euo pipefail
...
echo "integration: generator r2 local S3 (wrangler experimental)"
(
  cd "$root/apps/generator"
  # why: NI（./test/ LocalS3）が Start を共有するため peer_live 単独 package を二重起動しない。
