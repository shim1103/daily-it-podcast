...
- 2026-09-17 [feature/generator-textwriter-fallback] 秒数・文字数をtarget±marginの畳み込み定数で管理する設計では、定数変更時にそれを使うfixture・実例文（テスト用JSON等）の実際の文字数が新rangeに収まるかを個別に再検証する。定数だけ変えてfixtureを据え置くと、rangeを外れて既存testが壊れる  # → layer:terms
- 2026-09-17 [feature/generator-textwriter-fallback] prompt本文のplaceholder（`{{X_MIN}}`/`{{X_MAX}}`等）を担う値が固定値化・range撤廃された時、placeholder自体をprompt文言から削除し忘れると、置換関数が対応する置換を持たないため未置換のまま出力に残る。値の設計変更とplaceholder文言の同期を1つのconcernとして扱う  # → layer:terms
- 2026-09-17 [feature/generator-textwriter-fallback] `git stash`が禁止されたsandboxでcommitを`--split`する時、「commit単独でのbuild緑」を全パッケージ`go build ./...`で厳密検証できない（他の未commit変更が残るため）。この制約下では、依存の起点となるAPI signature変更（例: interfaceのmethod signature）を境に、その変更が波及する全消費者（呼び出し元・実装・test）を1つのconcernへまとめてcommitし、境界をpackage単位ではなく「signature変更の影響範囲」で引く  # → layer:workflow
- 2026-09-17 [feature/generator-textwriter-fallback] 別sessionの古いworktreeから大量のfileをまとめてコピーしてstacked PRの1つを組み立てる時、コピー元がbase branchで既に撤去済みの機能（この回はGoogle Drive/OAuth）への参照をまだ持つ古いsnapshotである可能性がある。コピー直後に`go vet -tags <全tag>`等の全量ビルドチェックを行い、コピー元にしか存在しないpackage importが紛れていないかをsource変更作業の前に検出する  # → layer:workflow
- 2026-09-17 [feature/generator-workflow-fallback-wiring] rate計測用workflowにfallback経路のcredentialを追加する前に、そのworkflowが呼ぶtestの実装が実際にfallback UseCase経由か単体API直接呼び出しかを確認する。decisionが「fallback経路の終端も検証する」と述べていても、test実装が意図的にfallback非経由（1 dispatch = 1 API等の設計）のままなら、workflow側にcredentialを追加しても実際には使われないdead参照になる  # → layer:workflow
- 2026-09-17 [feature/generator-workflow-fallback-wiring] code-first architecture diagram（生成script）や設計SSoT文書（層・依存・credential一覧を持つ文書）は、機能追加のたびに更新されるとは限らない。新しいfallback段・credential分離を実装した後、それらの文書に残る旧設計（単純化された1段経路、旧credential用途の説明）を検索して洗い出し、実装と同じcommit群で追従させる  # → layer:workflow
