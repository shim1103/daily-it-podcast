...
- **再編（Decision `2026-09-03T14-45-00` / `14-46-00` / `14-47-00` / `16-30-00`。executor 実装済み・未 commit）**:
  ...
  - **cron 週次 + dispatch は system test を 1 回ずつ通すだけ**（`speech_synthesis` 1 回版）。N 回ループも `-count=N` も入れない。壊れは 1 回で落ちる。
  - `gemini_excluded_full_system_test.go` / `produce_episode_system_test.go`(full) / `drive_observe.go` を削除。`//go:build system && full` 消滅。
  - PASS 率・所要の計測は dispatch 専用 test（`tts_rate`、`system && ratemeasure`）+ 専用 workflow に残す。**1 回通しが落ちた後の切り分け用**。env は `TEST_GEMINI_API_KEY` 直読み。
  - workflow の長い集計 shell を `scripts/generator/*-summary.sh` へ切り出し。
  - **Cursor は CLI（`agent` binary）→ Cloud Agents HTTP API（`manuscript/cursorapi`）へ移行済み（develop 取り込み、Decision `2026-09-03T17-03-33`）**。`cursorcli` / `commandlaunch` / `processenv` / `x` package と `install-cursor-cli.sh` / `cursorcli_draft_system_test.go` / `draft_rate_system_test.go` / `generator-draft-rate.yml` は削除。cursorapi 版の system 網羅（1 回通し・rate 計測）は未着手（別 Issue）。
...
5. `speech_synthesis_system_test.go`（`system`）= 実 GEMINI で topic+2 束を 1 回 `SynthesizeAll` へ渡し通す。（旧 `cursorcli_draft_system_test.go` は Cursor HTTP API 移行で削除。cursorapi 版の system 回帰 test は別 Issue。）
6. `build.ComposeBriefWithTemplate(items, template)` を分離（`ComposeBrief` は既定 template へ委譲、出力不変。`2026-09-03T14-47-00`）。
7. brief prompt に # Length strategy と # Self-check + parse 注意（`2026-09-02T18-26-00`）。Domain 定数不変。

## 次
1. `TEST_CURSOR_API_KEY` / `TEST_GEMINI_API_KEY` の repo Secret 登録（人手）。未登録だと TEST key 化後の実 API 部分が Skip / 失敗。
2. feature → develop PR。cron で走るのは `speech_synthesis`(1 回版) の 1 回通しのみ。cursorapi の system 網羅は別 Issue。
...
4. cron が赤／不安定なとき rate 計測を dispatch: `gh workflow run generator-tts-rate.yml [-f runs= -f call_gap= -f double=min|tgt|max]`。結果は `generator-system-pass-rate.md` へ記録。（draft rate は Cursor HTTP API 移行で workflow ごと削除。cursorapi 版の rate 計測は別 Issue。）
5. 3/3 の全体 3473 は下限 3360 まで 113 文字マージン。再発が続くなら prompt `const`（`constants.TextWriterBriefPrompt`）の detail 目安を上げる。
...
## memo
- `//go:build system` でビルドされる test file は `speech_synthesis_system_test.go` の 1 本。`system && ratemeasure` で `tts_rate` が加わる。
