...
このIssueでは、generator の Infrastructure に Google Drive API への書込 Driven Adapter を実装し、所定フォルダへ `{episodeId}.json` と `{episodeId}.wav` を置ける状態にする。完了後、Application 所有の書込 Port を本番 Adapter が満たし、HTTP Stub の Unit が Pass する。

## 2. Context

- Drive 向け Port・Adapter・秘密キー名は未実装。`PostSource` と `SpeechSynthesizer`（Gemini / WAV）は実装済み
- Fake Drive は本 Issue の対象外。playback の in-memory Fake とも共有しない
- 音声の正は `{episodeId}.wav`（`contracts/drive-layout.md`）。mp3 は使わない
- 仮定: Application 書込 Port が未存在のため、本 Issue で最小の書込 Port を Application に定義する。原稿生成 UseCase は別。入力音声は `SpeechAudio.Content`（WAV bytes）
- 仮定: 同一 `{episodeId}` の再書込は上書き。削除 API は扱わない
- 仮定: 認証は `DESIGN.md` の OAuth refresh。個別キー名が README に無い場合は名前行だけ足してよい（値は書かない）

## 3. Canonical Sources

- `contracts/drive-layout.md` — Drive 配置・命名・ペア（書込側が従う形。音声は wav。一覧読取は playback）
- `contracts/manuscript.schema.json` — 原稿 JSON
- `docs/decisions/2026-08-18T11-17-00-feature-tts-speech-synthesizer-wav.md` — TTS 戻りと保存形式は WAV。mp3 encoder 禁止
- `docs/decisions/2026-08-15T16-23-07-develop-drive-identity.md` — `episodeId` と `date` の分離
- `apps/generator/internal/entities/models/speech_audio.go` — 書込対象の音声 bytes
...
- Application 所有の書込 Port（名前は実装時。json + wav を所定フォルダへ書く操作）
...
| 操作 | 入力 | 成功 | 失敗 |
|---|---|---|---|
| 書込 | 非空 `episodeId`、原稿、WAV byte | 所定フォルダ直下に `{episodeId}.json` と `{episodeId}.wav` | JSON 不適合 / stem 不一致 → Domain。Drive I/O → Infrastructure |
...
- [ ] AC-1: Stub が Drive 成功を返すとき、json と wav が同じ stem でフォルダ直下へ書かれる
...
- 先行: `SpeechSynthesizer` / Gemini WAV Adapter（済）
- 並行可: Cursor CLI Infrastructure
- 後続: 原稿→TTS→書込を通す UseCase と `cmd` / GHA（本 Adapter を呼ぶ）
- 共有しない: `playback-worker-drive-adapter.md`（読取。言語・runtime・方向が違う）

## 10. Risks

- json 成功・wav 失敗で不完全ペアが残る → 全体を失敗とし、補償削除は後続に残す
