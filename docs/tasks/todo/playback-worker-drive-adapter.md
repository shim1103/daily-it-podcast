...
このIssueでは、worker Infrastructure に Google Drive API への読取 Driven Adapter を実装し、Application の Episode 読取 Port を本番 Drive で満たす。完了後、List / Get JSON / Get 音声が Drive HTTP Stub の Unit で Pass し、HTTP status や Route は知らない。

## 2. Context

- HTTP 契約は `apps/playback/contracts/` に固定済み
- Port / UseCase / in-memory Fake は `playback-worker-episodes.md` が所有。本 Issue は本番 Adapter のみ
- この worktree の `apps/playback/worker/` は未実装。episodes が先
- List は JSON 目録のみ。mp3 の有無は Get で見る（decision 参照）
...
- `contracts/drive-layout.md` — Drive 読取（一覧 `*.json`、1件は JSON + 音声）
...
- `docs/tasks/todo/playback-worker-episodes.md` — Port / UseCase / Fake の内側契約
...
- Google Drive API を使う Episode 読取 Port の本番実装
- 一覧: 所定フォルダ直下の `*.json`。音声の有無は見ない。schema 不適合・stem 不一致は行に出さない
- 1件: stem 一致の json + mp3。JSON 不適合・音声無しは Domain 不在
- 音声 byte。成功時の HTTP `Content-Type` は Route 側（本 Adapter は byte のみ）
...
- Port / UseCase / in-memory Fake（→ `playback-worker-episodes.md`）
...
Port の入出力は `playback-worker-episodes.md` §5 に従う。本 Issue はそれを Google Drive API で満たす。

**Drive 読取（`drive-layout.md`）**

- 一覧: `*.json` の stem = `episodeId`。音声は見ない
- 1件: stem 一致の json + mp3。不適合・欠落は返さない
...
- [ ] AC-3: json のみ（mp3 無し）の Get が Domain 不在になる
- [ ] AC-4: Get 成功時、返却原稿が `manuscript.schema.json` に適合する
- [ ] AC-5: Get 音声成功時、mp3 byte が取れる
...
- blocked by: `playback-worker-episodes.md`（Port + Fake。未実装なら先に終える）
...
- Fake と本番で「不在」判定がずれる → 判定は episodes の Port 契約に合わせ、Adapter は mechanism だけ持つ
