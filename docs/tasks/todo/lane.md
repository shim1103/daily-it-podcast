...
7. [ ] Access下の二段cache実測 — edge／browser／未認証がAccessで止まることを本番確認
8. [ ] PlaybackState.activeのepisodeId/audioRef非正規化 — Decision 2026-09-04 §1-1維持か正規化か
9. [ ] listEpisodesの原稿parse失敗の観測 — 除外warningの置き場を決める
10. [ ] 進捗clock skew秒 — 事実: 数値未決 / 次: 決定または仮値運用のまま落とすか
11. [ ] 進捗push／pull間隔 — 事実: 数値未決 / 次: sync Issueは仮値可、正本は後で
12. [ ] `durationSec < 3` 短尺の完走扱い — 事実: 未決 / 次: 定数化の要否
13. [ ] catalog寿命≠progress寿命／hook所有権 — 推測: hooks内で分かれうる / 次: 材料が足りたらDecision。公開面は凍らせない
