...
//
// @require httpClient != nil。cfg は config.Load で検証済みの R2Config（Config.R2）。logw != nil。
// @ensure 戻りは port.EpisodeWriter（検証後にだけ raw を呼ぶ）。
func newR2WriteEpisode(httpClient *http.Client, cfg config.R2Config, logw *delivery.LogWriter) port.EpisodeWriter {
...
func newR2CompletedEpisodeLookup(httpClient *http.Client, cfg config.R2Config, logw *delivery.LogWriter) port.CompletedEpisodeLookup {
