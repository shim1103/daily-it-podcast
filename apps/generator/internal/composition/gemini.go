...
//
// @require httpClient != nil。cfg.APIKey は検証済み。
// @ensure 戻りは port.SpeechSynthesizer。TierFree で構築する。TTS 1 呼び出しの長い timeout は Adapter が付け直す。
func newGeminiSpeechSynthesizerPrimary(httpClient *http.Client, cfg config.GeminiConfig) port.SpeechSynthesizer {
...
//
// @require httpClient != nil。cfg.SpareAPIKey は検証済み。
// @ensure 戻りは port.SpeechSynthesizer。TierPaid で構築する。TTS 1 呼び出しの長い timeout は Adapter が付け直す。
func newGeminiSpeechSynthesizerSpare(httpClient *http.Client, cfg config.GeminiConfig) port.SpeechSynthesizer {
	return gemini.NewSpeechSynthesizer(httpClient, cfg.SpareAPIKey.Reveal(), gemini.TierPaid)
}
