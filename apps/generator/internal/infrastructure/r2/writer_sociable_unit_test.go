...
func TestWrite_putsToEndpointBaseHost_whenWithEndpointBase(t *testing.T) {
	t.Parallel()

	// Given: endpoint base を上書きした Writer
	rt := &seqRoundTripper{resps: []stubResp{
		{Status: http.StatusOK},
		{Status: http.StatusOK},
	}}
	base := newStubWriter(rt)
	w := base.WithEndpointBase(" http://127.0.0.1:18787/cdn-cgi/local/r2/s3 ")

	// When: Write する
	if err := w.Write(context.Background(), "ep-base", []byte(`{}`), models.SpeechAudio{Content: []byte("a")}); err != nil {
		t.Fatalf("Write: %v", err)
	}

	// Then: override host へ put。元 Writer は本番 host のまま（受信者不変）
	if len(rt.calls) != 2 {
	...
	if !strings.Contains(rt.calls[0].Path, "/cdn-cgi/local/r2/s3/") {
		t.Fatalf("path = %q, want local S3 prefix", rt.calls[0].Path)
	}
	rt2 := &seqRoundTripper{resps: []stubResp{{Status: http.StatusOK}, {Status: http.StatusOK}}}
	orig := r2.NewEpisodeWriter(&http.Client{Transport: rt2}, testAccessKeyID, testSecretAccessKey, testAccountID, testBucket)
	_ = orig.WithEndpointBase("http://127.0.0.1:9/cdn-cgi/local/r2/s3")
	if err := orig.Write(context.Background(), "ep-orig", []byte(`{}`), models.SpeechAudio{Content: []byte("a")}); err != nil {
		t.Fatalf("orig Write: %v", err)
	}
	if rt2.calls[0].Host != testAccountID+".r2.cloudflarestorage.com" {
		t.Fatalf("orig host = %q, want production account host", rt2.calls[0].Host)
	}
}

func TestEpisodeWriter_WithEndpointBase_returnsNil_whenReceiverNil(t *testing.T) {
	t.Parallel()
	var w *r2.EpisodeWriter
	if got := w.WithEndpointBase("http://127.0.0.1:1/x"); got != nil {
