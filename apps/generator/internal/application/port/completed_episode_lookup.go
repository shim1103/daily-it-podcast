...
// storage 配置・vendor 固有 id・MIME は Infrastructure に閉じる。
//
// @require date は YYYY-MM-DD 暦日文字列。
// @ensure 同一 stem の {stem}.json と {stem}.mp3 が両方あり、かつ json の date field が date と一致するとき true。
// @ensure 片方だけ・無し・date 不一致は false（error ではない）。
// @invariant stem・vendor 固有 id・MIME・vendor 型を露出しない。method は HasPair のみ。
