...
// 実物境界: publickey.ListItemSource が標準 *http.Client で到達する本番 Publickey Atom feed
...
// @ensure 各要素の SourceID は publickey.SourceID。OccurredAt は UTC かつ since 以上。
...
func TestPublickeyListItemSource_reachesRealBoundary_whenAuthFreeGetSucceeds(t *testing.T) {
	// Given: 本番 feed 向け標準 client と、空日 flake を抑える広い since 窓
	since := time.Now().UTC().Add(-30 * 24 * time.Hour)
	source := publickey.NewListItemSource(newItemSourceHTTPClient(60 * time.Second))

	// When: 実境界へ List する
	got, err := source.List(context.Background(), since)

	// Then: I/O 契約（成功・非 nil・要素があれば SourceID / OccurredAt）
	assertRealBoundaryItemSourceList(t, got, err, publickey.SourceID, since)
}
