...
// 実物境界: lobsters.ListItemSource が標準 *http.Client で到達する本番 Lobsters JSON API
...
// @ensure 各要素の SourceID は lobsters.SourceID。OccurredAt は UTC かつ since 以上。
...
func TestLobstersListItemSource_reachesRealBoundary_whenAuthFreeGetSucceeds(t *testing.T) {
	// Given: 本番 API 向け標準 client と、空日 flake を抑える広い since 窓
	since := time.Now().UTC().Add(-30 * 24 * time.Hour)
	source := lobsters.NewListItemSource(newItemSourceHTTPClient(60 * time.Second))

	// When: 実境界へ List する
	got, err := source.List(context.Background(), since)

	// Then: I/O 契約（成功・非 nil・要素があれば SourceID / OccurredAt）
	assertRealBoundaryItemSourceList(t, got, err, lobsters.SourceID, since)
}
