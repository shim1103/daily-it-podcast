...
export type EpisodeListItemData = EpisodeData;
...
/**
 * 一覧 page が使う state（loading / success / error）と選択中 episode の詳細を持つ ViewModel hook。
 *
 ...
 *   同じ episodeId が選択中なら選択を解除し、それ以外は一覧から lookup して選択する。
...
export function useEpisodeListViewModel(apiClient: PlaybackApiClient): EpisodeListViewModel {
  ...
  const select = useCallback(
    async (episodeId: string): Promise<void> => {
      ...
      const episode = current.episodes.find((item) => item.episodeId === episodeId);
      ...
      setState({
        ...current,
        selectedEpisodeId: episodeId,
        selectedEpisode: { status: "success", episode },
      });
    },
    [setState],
  );
