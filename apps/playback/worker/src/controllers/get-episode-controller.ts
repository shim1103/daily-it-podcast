...
export type GetEpisodeUseCase = (
  episodeId: string,
) => Promise<GetEpisodeResponse>;

export type GetEpisodeController = (
  body: unknown,
) => Promise<GetEpisodeResponse>;
 ...
 */
export function createGetEpisodeController(
  useCase: GetEpisodeUseCase,
): GetEpisodeController {
  return async function getEpisodeController(
    body: unknown,
  ): Promise<GetEpisodeResponse> {
