...
/**
 * 所定フォルダの原稿 json / mp3 を「取得したまま返す」Driven Port。schema 検証・stem 一致判定は
 ...
 * 不正 JSON・mp3 欠落の判定は use-case（`application/use-cases/*`）が `application/manuscript` の
 * 純関数を使って行う。
 *
...
export interface EpisodeRepository {
  ...
  /**
   * 対象 episodeId の mp3 byte を取得したまま返す。
   *
   * @ensure mp3 エントリまたは byte が無い時は `undefined` を返す（throw しない）。
